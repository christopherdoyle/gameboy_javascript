...
var opcode_ld_nnn = { // load 16 bit NN into reg n. CYCLES = 12
  compute: function(highbit) {
    if (highbit == 0x3) {
      hardware.cpu_registers[8] = hardware.memory[hardware.cpu_registers[9] + 1] 
      | (hardware.memory[hardware.cpu_registers[9] + 2] << 8);
    } else {
      hardware.cpu_registers[(highbit * 2) + 3] = hardware.memory[hardware.cpu_registers[9] + 1]; // lowbit
      hardware.cpu_registers[(highbit * 2) + 2] = hardware.memory[hardware.cpu_registers[9] + 2]; // highbit
    }
    hardware.cpu_registers[9] += 3;
  }
};

var lowbit_convert = function(lowbit) {
...
var opcode_arithmetic = {
  add_An: function(low) { // add n to A 
    var ni = lowbit_convert(low);
    var n = (ni == 67) ? hardware.get_full_registers(6) : hardware.cpu_registers[n];
    var A = hardware.cpu_registers[0]; // store for checking
    if (n != 67) {
  ...
  inc8bit: function(opcode) {
    var n = lowbit_convert((opcode - 4) / 8);
    if (n == 67) {
...
var start = function(prog_data) {
  while (true) {
        console.log(hardware.cpu_registers[9].toString(16) + " > " + hardware.memory[hardware.cpu_registers[9]].toString(16));
